...
int getNumPlayers() {
    ...
    try {
        ...
        // do nothing, uin being empty will throw the second else if cout
    }
    while (nP < 3 || nP > 10) {
        cout << "Invalid input, please try again: ";
        getline(cin, numPlayers);
...
void firstMove(Deck *deck, int playerNumber) {
    ...
    for (int i = 0; i < deck->players[playerNumber].playerCards.size(); i++) {
        ...
        char key = '0' + (i+1);
        cardOptions[key] = deck->players[playerNumber].playerCards[i];
    }
    ...
    while(true) {
        ...
        if (userInput == "done") {
            ...
            if(std::find(alreadyChosen.begin(), alreadyChosen.end(), uin) != alreadyChosen.end()) {
                cout << "You must enter a number corresponding to a card not previously chosen"<< endl;
            }
            else if (uin < 1 || uin > cardOptionsSize) {
                cout << "You must enter a number corresponding to a card or 'done'"<< endl;
            }
            else {
                cardsIn++;
                alreadyChosen.push_back(uin);
                char userIn = '0' + uin;
                deck->pile.push_back(cardOptions[userIn]);
                deck->prevCards.push_back(cardOptions[userIn]);
                cardOptions.erase(userIn);
                deck->players[playerNumber].playerCards.erase(deck->players[playerNumber].playerCards.begin() + uin-1-alreadyChosen.size());
            }
        }
